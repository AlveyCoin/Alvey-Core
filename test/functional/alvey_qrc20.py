...
from test_framework.alvey import *
...
arc20burn
arc20burnfrom
arc20listtransactions
"""


class AlveyCoinARC20Test(BitcoinTestFramework):
    ...
    def arc20name_test(self):
        assert_equal(self.node.arc20name(self.contract_address), "LRC TEST")

    def arc20symbol_test(self):
        assert_equal(self.node.arc20symbol(self.contract_address), "QTC")

    def arc20totalsupply_test(self):
        assert_equal(self.node.arc20totalsupply(self.contract_address), f"{10**65}.00000000")

    def arc20decimals_test(self):
        assert_equal(self.node.arc20decimals(self.contract_address), 8)
    ...
    def run_test(self):
        ...
        """
            ...
            /**
                ARC20Token Standard Token implementation
            */
        ...
        self.arc20name_test()
        self.arc20symbol_test()
        self.arc20totalsupply_test()
        self.arc20decimals_test()
        assert_equal(self.node.arc20balanceof(self.contract_address, self.creator), f"{10**65}.00000000")

        disconnect_nodes(self.node, 1)
        for n in self.nodes:
            assert_equal(n.getpeerinfo(), [])

        self.node.arc20transfer(self.contract_address, self.creator, self.receiver, "0.00000001")
        self.node.generate(1)
        tip = self.node.getblock(self.node.getbestblockhash())
        assert_equal(self.node.arc20listtransactions(self.contract_address, self.creator, 0, 0), 
            [{
        ...
        assert_equal(self.node.arc20listtransactions(self.contract_address, self.receiver, 0, 0), 
            [{
                'receiver': self.receiver, 
                'sender': self.creator,
                'amount': '0.00000001', 
        ...
        assert_equal(self.node.arc20balanceof(self.contract_address, self.creator), f"{10**65-1}.99999999")
        assert_equal(float(self.node.arc20balanceof(self.contract_address, self.receiver)), 0.00000001)


        # Double spend the arc20 transaction to clear it from the mempool and reorg to clear the state
        ...
        assert_equal(self.node.arc20listtransactions(self.contract_address, self.creator, 0, 0), [])
        assert_equal(self.reorg_node.arc20listtransactions(self.contract_address, self.creator, 0, 0), [])
        assert_equal(self.reorg_node.arc20balanceof(self.contract_address, self.creator), f"{10**65}.00000000")
        assert_equal(self.node.arc20balanceof(self.contract_address, self.creator), f"{10**65}.00000000")
        assert_equal(float(self.reorg_node.arc20balanceof(self.contract_address, self.receiver)), 0)
        assert_equal(float(self.node.arc20balanceof(self.contract_address, self.receiver)), 0)
        ...
        self.node.arc20transfer(self.contract_address, self.creator, self.receiver, f"{10**65}.00000000")
        self.node.generate(1)
        #sync_blocks(self.nodes)
        tip = self.node.getblock(self.node.getbestblockhash())
        assert_equal(self.node.arc20listtransactions(self.contract_address, self.receiver, 0, 0), 
        ...
        assert_raises_rpc_error(-1, "Not enough token balance", self.reorg_node.arc20transfer, self.contract_address, self.receiver, self.creator, "0.1")

        # Make sure that sending from an address that we do not control fails gracefully
        assert_raises_rpc_error(-4, "Private key not available", self.reorg_node.arc20transfer, self.contract_address, self.creator, self.receiver, "0.1")

        # Make sure that the allowance functionality works as intended
        self.approve_tx_receiver = self.node.getnewaddress()
        self.node.arc20approve(self.contract_address, self.creator, self.receiver, "0.1")
        self.node.generatetoaddress(1, self.creator)
        sync_blocks(self.nodes)

        assert_equal(float(self.node.arc20allowance(self.contract_address, self.creator, self.receiver)), 0.1)
        assert_raises_rpc_error(-1, "Not enough token allowance", self.reorg_node.arc20transferfrom, self.contract_address, self.creator, self.receiver, self.approve_tx_receiver, "0.10000001")

        self.reorg_node.arc20transferfrom(self.contract_address, self.creator, self.receiver, self.approve_tx_receiver, "0.01")
        self.sync_all()
        self.node.generatetoaddress(1, self.creator)
        sync_blocks(self.nodes)
        tip = self.node.getblock(self.node.getbestblockhash())
        assert_equal(self.reorg_node.arc20listtransactions(self.contract_address, self.approve_tx_receiver, 0, 0), [{
            'amount': '0.01000000',
            'blockHash': tip['hash'],
            'blockNumber': 2107,
            'blocktime': tip['time'],
        ...
        assert_raises_rpc_error(-1, "Not enough token allowance", self.reorg_node.arc20transferfrom, self.contract_address, self.creator, self.receiver, self.approve_tx_receiver, "0.09000001")

        self.reorg_node.arc20transferfrom(self.contract_address, self.creator, self.receiver, self.approve_tx_receiver, "0.09000000")
        ...
        assert_equal(float(self.node.arc20allowance(self.contract_address, self.creator, self.receiver)), 0)
        assert_equal(self.reorg_node.arc20balanceof(self.contract_address, self.creator), f"{10**65-1}.90000000")
        assert_equal(self.node.arc20balanceof(self.contract_address, self.approve_tx_receiver), f"0.10000000")




if __name__ == '__main__':
    AlveyCoinARC20Test().main()
