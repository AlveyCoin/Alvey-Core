...
from .alveyconfig import COINBASE_MATURITY
from .alvey import generatesynchronized
...
class BitcoinTestFramework(metaclass=BitcoinTestMetaClass):
    ...
    def setup(self):
        ...
        self.config = config
        self.options.bitcoind = os.getenv("BITCOIND", default=config["environment"]["BUILDDIR"] + '/src/alveyd' + config["environment"]["EXEEXT"])
        self.options.bitcoincli = os.getenv("BITCOINCLI", default=config["environment"]["BUILDDIR"] + '/src/alvey-cli' + config["environment"]["EXEEXT"])
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            for entry in os.listdir(cache_path()):
                if entry not in ['chainstate', 'blocks', 'stateAlveyCoin']:  # Only keep chainstate and blocks folder
