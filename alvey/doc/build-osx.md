...
This guide describes how to build alveyd, command-line utilities, and GUI on macOS
...
These tools must be installed in order to build Alvey Core from source.
...
### 4. Clone Alvey repository

`git` should already be installed by default on your system.
Now that all the required dependencies are installed, let's clone the Alvey Core repository to a directory.
...
It is not necessary to build wallet functionality to run `alveyd` or  `alvey-qt`.
...
Alvey Core includes a GUI built with the cross-platform Qt Framework.
...
You can deploy a `.dmg` containing the Alvey Core application using `make deploy`.
...
There are many ways to configure Alvey Core, here are a few common examples:
...
Run the following in your terminal to compile Alvey Core:
...
Alvey Core should now be available at `./src/alveyd`.
If you compiled support for the GUI, it should be available at `./src/qt/alvey-qt`.

The first time you run `alveyd` or `alvey-qt`, it will start downloading the blockchain.
This process could take many hours, or even days on slower than average systems.

By default, blockchain and wallet data files will be stored in:

``` bash
/Users/${USER}/Library/Application Support/Alvey/
```

Before running, you may create an empty configuration file:

```shell
mkdir -p "/Users/${USER}/Library/Application Support/Alvey"

touch "/Users/${USER}/Library/Application Support/Alvey/alvey.conf"

chmod 600 "/Users/${USER}/Library/Application Support/Alvey/alvey.conf"
```

You can monitor the download process by looking at the debug.log file:

```shell
tail -f $HOME/Library/Application\ Support/Alvey/debug.log
```

## Other commands:

```shell
./src/alveyd -daemon      # Starts the alvey daemon.
./src/alvey-cli --help    # Outputs a list of command-line options.
./src/alvey-cli help      # Outputs a list of RPC commands when the daemon is running.
./src/qt/alvey-qt -server # Starts the alvey-qt server mode, allows alvey-cli control
```
