Gitian building
================
*Setup instructions for a Gitian build of Alvey Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Alvey
...
- [Building Alvey Core](#building-alvey-core)
...
Clone the git repositories for alvey and Gitian.
...
```bash
sudo cp /home/debian/alvey/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the alvey repository under 'Fetch and create inputs' to install sources which require
...
To build Alvey Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the alvey repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/alvey/.git/
...
For example:
```bash
URL=https://github.com/laanwj/alvey.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit alvey=${COMMIT} --url alvey=${URL} ../alvey/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit alvey=${COMMIT} --url alvey=${URL} ../alvey/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit alvey=${COMMIT} --url alvey=${URL} ../alvey/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-alvey:
./bin/gbuild --commit alvey=${COMMIT},cpp-eth-alvey=${ETHCOMMIT} --url alvey=${URL},cpp-eth-alvey=${ETHURL} ../alvey/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the alvey git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../alvey/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/alveycoin/alvey-detached-sigs.git

BTCPATH=/some/root/path/alvey
SIGPATH=/some/root/path/alvey-detached-sigs

./bin/gbuild --url alvey=${BTCPATH},signature=${SIGPATH} ../alvey/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/alvey-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/alvey-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/alvey-osx-build.assert
```
...
[alveycoin/gitian.sigs](https://github.com/alveycoin/gitian.sigs/) repository, or if that's not possible create a pull
