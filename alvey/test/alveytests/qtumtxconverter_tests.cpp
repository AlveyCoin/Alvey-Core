...
void checkResult(bool isCreation, std::vector<AlveyTransaction> results, uint256 hash){
...
void runTest(CChainState& chainstate, CTxMemPool& mempool, bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    AlveyTxConverter converter(transaction, chainstate, &mempool, NULL);
    ExtractAlveyTX alveyTx;
    BOOST_CHECK(converter.extractionAlveyTransactions(alveyTx));
    std::vector<AlveyTransaction> result = alveyTx.first;
    if(script2 == CScript()){
