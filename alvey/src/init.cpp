 ...
 */
static const char* BITCOIN_PID_FILENAME = "alveyd.pid";
...
void Shutdown(NodeContext& node)
{
    ...
    util::ThreadRename("alvey-shutoff");
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/alveycoin/alvey>";
...
void DeleteBlockChainData()
{
    ...
    fs::remove_all(datadir / "stateAlvey");
...
bool AppInitMain(NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if (args.IsArgSet("-datadir") && !fs::path(args.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if alvey is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if alvey is started while in a temporary directory.\n",
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                fs::path alveyStateDir = gArgs.GetDataDirNet() / "stateAlvey";
                bool fStatus = fs::exists(alveyStateDir);
                const std::string dirAlvey(alveyStateDir.string());
                const dev::h256 hashDB(dev::sha3(dev::rlp("")));
                dev::eth::BaseState existsAlveystate = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
                globalState = std::unique_ptr<AlveyState>(new AlveyState(dev::u256(0), AlveyState::openDB(dirAlvey, hashDB, dev::WithExisting::Trust), dirAlvey, existsAlveystate));
                globalSealEngine = std::unique_ptr<dev::eth::SealEngineFace>(cp.createSealEngine());

                pstorageresult.reset(new StorageResults(alveyStateDir.string()));
                if (fReset) {
            ...
            try {
                ...
                globalSealEngine->setAlveySchedule(alveyDGP.getGasSchedule(active_chain.Height() + (active_chain.Height()+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1) ));
