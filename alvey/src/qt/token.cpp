...
Token::Token()
{
    ...
    lstMandatory.append(AlveyToken::paramAddress());
    lstMandatory.append(AlveyToken::paramDatahex());
    QStringList lstOptional;
    lstOptional.append(AlveyToken::paramSender());
    ...
    lstMandatory.append(AlveyToken::paramDatahex());
    lstOptional.clear();
    lstOptional.append(AlveyToken::paramAmount());
    lstOptional.append(AlveyToken::paramGasLimit());
    lstOptional.append(AlveyToken::paramGasPrice());
    lstOptional.append(AlveyToken::paramSender());
    lstOptional.append(AlveyToken::paramBroadcast());
    lstOptional.append(AlveyToken::paramChangeToSender());
    lstOptional.append(AlveyToken::paramPsbt());
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            if(numTopics > 1)
            {
                ...
                Token::ToAlveyAddress(tokenEvent.sender, tokenEvent.sender);
            }
            if(numTopics > 2)
            {
                tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
                Token::ToAlveyAddress(tokenEvent.receiver, tokenEvent.receiver);
            }
