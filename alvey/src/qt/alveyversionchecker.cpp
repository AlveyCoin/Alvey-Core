...
#define paternVersion "alvey-([0-9]+\\.)?([0-9]+\\.)?([0-9]+)-"

AlveyVersionChecker::AlveyVersionChecker(QObject *parent) : QObject(parent)
{
    currentVersion = Version(CLIENT_VERSION_MAJOR, CLIENT_VERSION_MINOR, 0);
}

AlveyVersionChecker::~AlveyVersionChecker()
{

}

bool AlveyVersionChecker::newVersionAvailable()
{
    Version maxReleaseVersion = getMaxReleaseVersion();
    return maxReleaseVersion > currentVersion;
}

QList<Version> AlveyVersionChecker::getVersions()
{
...
Version AlveyVersionChecker::getMaxReleaseVersion()
{
