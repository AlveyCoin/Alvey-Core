...
#include <qt/alveyversionchecker.h>
    ...
    m_network_style(networkStyle)
{
    ...
    alveyVersionChecker = new AlveyVersionChecker(this);
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Alvey address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and alvey: URIs)"));
    ...
    ARCTokenAction = new QAction(platformStyle->MultiStatesIcon(":/icons/arctoken"), tr("&QRC Tokens"), this);
    ARCTokenAction->setStatusTip(tr("QRC Tokens (send, receive or add Tokens in list)"));
    ARCTokenAction->setToolTip(ARCTokenAction->statusTip());
    ARCTokenAction->setCheckable(true);
    ARCTokenAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_5));
    tabGroup->addAction(ARCTokenAction);
    ...
    connect(ARCTokenAction, SIGNAL(triggered()), this, SLOT(showNormalIfMinimized()));
    connect(ARCTokenAction, SIGNAL(triggered()), this, SLOT(gotoTokenPage()));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Alvey addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Alvey addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Alvey Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Alvey Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a alvey: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Alvey command-line options").arg(PACKAGE_NAME));
...
void BitcoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        appNavigationBar->addAction(ARCTokenAction);
...
void BitcoinGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        if(_clientModel->getOptionsModel()->getCheckForUpdates() && alveyVersionChecker->newVersionAvailable())
        {
            ...
            QString message(tr("New version of Alvey wallet is available on the Alvey source code repository: <br /> %1. <br />It is recommended to download it and update this application").arg(link));
...
void BitcoinGUI::setWalletActionsEnabled(bool enabled)
{
    ...
    ARCTokenAction->setEnabled(enabled);
...
{
    ARCTokenAction->setChecked(true);
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Alvey network.", "", count);
    } else {
        //: A substring of the tooltip.
