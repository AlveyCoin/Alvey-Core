...
class AlveyHwiToolPriv
{
public:
    AlveyHwiToolPriv(QObject *parent)
    {
...
AlveyHwiTool::AlveyHwiTool(QObject *parent) : QObject(parent)
{
    d = new AlveyHwiToolPriv(this);
}

AlveyHwiTool::~AlveyHwiTool()
{
    delete d;
}

bool AlveyHwiTool::enumerate(QList<HWDevice> &devices, bool stake)
{
    LOCK(cs_ledger);
    devices.clear();
    std::vector<LedgerDevice> vecDevices;
    if(AlveyLedger::instance().enumerate(vecDevices, stake))
    {
    ...
    {
        d->strError = QString::fromStdString(AlveyLedger::instance().errorMessage());
    }

    return devices.size() > 0;
}

bool AlveyHwiTool::isConnected(const QString &fingerprint, bool stake)
{
    LOCK(cs_ledger);
    std::string strFingerprint = fingerprint.toStdString();
    bool ret = AlveyLedger::instance().isConnected(strFingerprint, stake);
    if(!ret) d->strError = QString::fromStdString(AlveyLedger::instance().errorMessage());
    return ret;
}

bool AlveyHwiTool::getKeyPool(const QString &fingerprint, int type, const QString& path, bool internal, QString &desc)
{
    LOCK(cs_ledger);
    std::string strFingerprint = fingerprint.toStdString();
    ...
    bool ret = AlveyLedger::instance().getKeyPool(strFingerprint, type, strPath, internal, d->from, d->to, strDesc);
...
bool AlveyHwiTool::getKeyPool(const QString &fingerprint, int type, const QString &path, QStringList &descs)
{
    LOCK(cs_ledger);
    bool ret = true;
    QString desc;
...
bool AlveyHwiTool::getKeyPoolPKH(const QString &fingerprint, const QString& path, QStringList &descs)
{
    return getKeyPool(fingerprint, (int)OutputType::LEGACY, path, descs);
}

bool AlveyHwiTool::getKeyPoolP2SH(const QString &fingerprint, const QString& path, QStringList &descs)
{
    return getKeyPool(fingerprint, (int)OutputType::P2SH_SEGWIT, path, descs);
}

bool AlveyHwiTool::getKeyPoolBech32(const QString &fingerprint, const QString& path, QStringList &descs)
{
    return getKeyPool(fingerprint, (int)OutputType::BECH32, path, descs);
}

bool AlveyHwiTool::signTx(const QString &fingerprint, QString &psbt)
{
    LOCK(cs_ledger);
    std::string strFingerprint = fingerprint.toStdString();
    std::string strPsbt = psbt.toStdString();
    bool ret = AlveyLedger::instance().signTx(strFingerprint, strPsbt);
    psbt = QString::fromStdString(strPsbt);
    if(!ret) d->strError = QString::fromStdString(AlveyLedger::instance().errorMessage());
    return ret;
}

bool AlveyHwiTool::signMessage(const QString &fingerprint, const QString &message, const QString &path, QString &signature)
{
    LOCK(cs_ledger);
    std::string strFingerprint = fingerprint.toStdString();
    ...
    bool ret = AlveyLedger::instance().signMessage(strFingerprint, strMessage, strPath, strSignature);
    signature = QString::fromStdString(strSignature);
    if(!ret) d->strError = QString::fromStdString(AlveyLedger::instance().errorMessage());
    return ret;
}

bool AlveyHwiTool::signDelegate(const QString &fingerprint, QString &psbt)
{
...
QString AlveyHwiTool::errorMessage()
{
...
bool AlveyHwiTool::isStarted()
{
    return d->fStarted;
}

void AlveyHwiTool::wait()
{
    if(d->fStarted)
    {
...
bool AlveyHwiTool::rescanBlockchain(int startHeight, int stopHeight)
{
    if(!d->model) return false;
...
bool AlveyHwiTool::importAddresses(const QString &desc)
{
    if(!d->model) return false;
...
bool AlveyHwiTool::importMulti(const QStringList &descs)
{
    bool ret = true;
...
bool AlveyHwiTool::finalizePsbt(const QString &psbt, QString &hexTx, bool &complete)
{
    if(!d->model) return false;
...
bool AlveyHwiTool::sendRawTransaction(const QString &hexTx, QVariantMap& variantMap)
{
    if(!d->model) return false;
...
bool AlveyHwiTool::decodePsbt(const QString &psbt, QString &decoded)
{
    if(!d->model) return false;
...
void AlveyHwiTool::setModel(WalletModel *model)
{
    d->model = model;
}

bool AlveyHwiTool::execRPC(ExecRPCCommand *cmd, const QMap<QString, QString> &lstParams, QVariant &result, QString &resultJson)
{
    d->strError.clear();
...
void AlveyHwiTool::addError(const QString &error)
{
    if(d->strError != "")
        d->strError += "\n";
    d->strError += error;
...
QString AlveyHwiTool::derivationPathPKH()
{
    std::string path = AlveyLedger::instance().derivationPath((int)OutputType::LEGACY);
    return QString::fromStdString(path);
}

QString AlveyHwiTool::derivationPathP2SH()
{
    std::string path = AlveyLedger::instance().derivationPath((int)OutputType::P2SH_SEGWIT);
    return QString::fromStdString(path);
}

QString AlveyHwiTool::derivationPathBech32()
{
    std::string path = AlveyLedger::instance().derivationPath((int)OutputType::BECH32);
    return QString::fromStdString(path);
}
