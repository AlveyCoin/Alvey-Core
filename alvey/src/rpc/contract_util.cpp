...
UniValue transactionReceiptToJSON(const AlveyTransactionReceipt& txRec)
{
    UniValue result(UniValue::VOBJ);
...
UniValue CallToContract(const UniValue& params, ChainstateManager &chainman)
{
    ...
    if(!params[2].isNull()){
        ...
        if (IsValidDestination(alveySenderAddress)) {
            PKHash keyid = std::get<PKHash>(alveySenderAddress);
    ...
    for(size_t i = 0; i < list.size(); i++)
    {
        ...
        for(size_t i = 0; i < listLog.size(); i++)
        {
            ...
            if(numTopics > 1)
            {
                ...
                ToAlveyAddress(tokenEvent.sender, tokenEvent.sender);
            }
            if(numTopics > 2)
            {
                tokenEvent.receiver = topicsList[2].get_str().substr(24);
                ToAlveyAddress(tokenEvent.receiver, tokenEvent.receiver);
            }
