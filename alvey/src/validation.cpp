...
std::unique_ptr<AlveyState> globalState;
...
bool fAddressIndex = false; // alvey
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if(tx.HasCreateOrCall()){
        ...
        AlveyDGP alveyDGP(globalState.get(), m_active_chainstate, fGettingValuesDGP);
        uint64_t minGasPrice = alveyDGP.getMinGasPrice(m_active_chainstate.m_chain.Tip()->nHeight + 1);
        uint64_t blockGasLimit = alveyDGP.getBlockGasLimit(m_active_chainstate.m_chain.Tip()->nHeight + 1);
        size_t count = 0;
        ...
        AlveyTxConverter converter(tx, m_active_chainstate, &m_pool, NULL, NULL, contractflags);
        ExtractAlveyTX resultConverter;
        if(!converter.extractionAlveyTransactions(resultConverter)){
        ...
        std::vector<AlveyTransaction> alveyTransactions = resultConverter.first;
        std::vector<EthTransactionParams> alveyETP = resultConverter.second;

        dev::u256 sumGas = dev::u256(0);
        dev::u256 gasAllTxs = dev::u256(0);
        for(const AlveyTransaction& alveyTransaction : alveyTransactions){
            sumGas += alveyTransaction.gas() * alveyTransaction.gasPrice();
            ...
            if(txMinGasPrice != 0) {
                txMinGasPrice = std::min(txMinGasPrice, alveyTransaction.gasPrice());
            } else {
                txMinGasPrice = alveyTransaction.gasPrice();
            }
            VersionVM v = alveyTransaction.getVersion();
            if(v.format!=0)
            ...
            if(alveyTransaction.gas() < gArgs.GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
            ...
            if(alveyTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
            ...
            if(alveyTransaction.gas() > UINT32_MAX)
            ...
            gasAllTxs += alveyTransaction.gas();
            ...
            if(v.rootVM!=0 && (uint64_t)alveyTransaction.gasPrice() < minGasPrice)
...
bool MemPoolAccept::Finalize(const ATMPArgs& args, Workspace& ws)
{
    ...
    for (CTxMemPool::txiter it : allConflicting)
    {
        LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s ALV additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    CAmount nSubsidy = 128 * COIN / blocktimeDownscaleFactor;
...
DisconnectResult CChainState::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // alvey
    globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // alvey
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, CChainState& chainstate, CBlockIndex* pblockindex, const dev::Address& sender, uint64_t gasLimit, CAmount nAmount)
{
    ...
    AlveyDGP alveyDGP(globalState.get(), chainstate, fGettingValuesDGP);
    uint64_t blockGasLimit = alveyDGP.getBlockGasLimit(pblockindex->nHeight + 1);
    ...
    AlveyTransaction callTransaction;
    if(addrContract == dev::Address())
    {
        callTransaction = AlveyTransaction(nAmount, 1, dev::u256(gasLimit), opcode, nonce);
    }
    else
    {
        callTransaction = AlveyTransaction(nAmount, 1, dev::u256(gasLimit), addrContract, opcode, nonce);
    }
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());


    ByteCodeExec exec(block, std::vector<AlveyTransaction>(1, callTransaction), blockGasLimit, pblockindex, chainstate.m_chain);
...
void writeVMlog(const std::vector<ResultExecute>& res, CChain& chain, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path alveyDir = gArgs.GetDataDirNet() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(alveyDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(alveyDir.string(), std::ios::in | std::ios::out);
    ...
    for(AlveyTransaction& tx : txs){
        ...
        if(!tx.isCreation() && !globalState->addressInUse(tx.receiveAddress())){
            ...
            result.push_back(ResultExecute{
                execRes,
                AlveyTransactionReceipt(dev::h256(), dev::h256(), dev::u256(), dev::eth::LogEntries(), {}, {}),
                CTransaction()
            });
            continue;
        }
...
bool AlveyTxConverter::extractionAlveyTransactions(ExtractAlveyTX& alveytx){
    ...
    std::vector<AlveyTransaction> resultTX;
    ...
    alveytx = std::make_pair(resultTX, resultETP);
    return true;
}

bool AlveyTxConverter::receiveStack(const CScript& scriptPubKey){
    sender = false;
...
bool AlveyTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
AlveyTransaction AlveyTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    AlveyTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = AlveyTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = AlveyTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
...
size_t AlveyTxConverter::correctedStackSize(size_t size){
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // alvey
    AlveyDGP alveyDGP(globalState.get(), *this, fGettingValuesDGP);
    globalSealEngine->setAlveySchedule(alveyDGP.getGasSchedule(pindex->nHeight + (pindex->nHeight+1 >= m_params.GetConsensus().QIP7Height ? 0 : 1) ));
    uint32_t sizeBlockDGP = alveyDGP.getBlockSize(pindex->nHeight + (pindex->nHeight+1 >= m_params.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t minGasPrice = alveyDGP.getMinGasPrice(pindex->nHeight + (pindex->nHeight+1 >= m_params.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t blockGasLimit = alveyDGP.getBlockGasLimit(pindex->nHeight + (pindex->nHeight+1 >= m_params.GetConsensus().QIP7Height ? 0 : 1));
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // alvey
...
///////////////////////////////////////////////////////////////////////////////////////// alvey
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            AlveyTxConverter convert(tx, *this, m_mempool, &view, &block.vtx, contractflags);

            ExtractAlveyTX resultConvertAlveyTX;
            if(!convert.extractionAlveyTransactions(resultConvertAlveyTX)){
            ...
            if(!CheckMinGasPrice(resultConvertAlveyTX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertAlveyTX.first, blockGasLimit, pindex->pprev, m_chain);
            ...
            for(AlveyTransaction& qtx : resultConvertAlveyTX.first){
            ...
            if (fLogEvents && !fJustCheck)
            {
                ...
                for(size_t k = 0; k < resultConvertAlveyTX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{
                        ...
                        tx.GetHash(),
                        uint32_t(i),
                        resultConvertAlveyTX.first[k].from(),
                        resultConvertAlveyTX.first[k].to(),
                        ...
                        resultConvertAlveyTX.first[k].getNVout(),
...
//////////////////////////////////////////////////////////////////
    ...
    if(pindex->nHeight > 5000 && !m_params.MineBlocksOnDemand()) {
        ...
        if(pindex->nMoneySupply > (uint64_t)(160000000 + ((pindex->nHeight - 5000) * 128)) * COIN){
...
bool CChainState::ConnectTip(BlockValidationState& state, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions& disconnectpool)
{
    ...
    LogPrint(BCLog::BENCH, "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * MILLI, nTimeReadFromDisk * MICRO);
    {
        ...
        dev::h256 oldHashStateRoot(globalState->rootHash()); // alvey
        dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // alvey
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot); // alvey
            globalState->setRootUTXO(oldHashUTXORoot); // alvey
...
bool SignBlockHWI(std::shared_ptr<CBlock> pblock, CWallet& wallet, std::vector<unsigned char>& vchSig)
{
    ...
    AlveyLedger &device = AlveyLedger::instance();
...
bool SignBlockLedger(std::shared_ptr<CBlock> pblock, CWallet& wallet)
{
    ...
    {
        std::string errorMessage = AlveyLedger::instance().errorMessage();
...
//////////////////////////////////////////////////////////////////////////
    ...
    for (pindex = chainstate.m_chain.Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        ...
        ///////////////////////////////////////////////////////////////////// // alvey
        uint32_t sizeBlockDGP = alveyDGP.getBlockSize(pindex->nHeight);
        dgpMaxBlockSize = sizeBlockDGP ? sizeBlockDGP : dgpMaxBlockSize;
        updateBlockSizeParams(dgpMaxBlockSize);
        /////////////////////////////////////////////////////////////////////
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for Alvey PoS we need to sync this after every block to ensure txdb is populated for
...
CAmount GetTxGasFee(const CMutableTransaction& _tx, const CTxMemPool& mempool, CChainState& active_chainstate)
{
    ...
    if(tx.HasCreateOrCall())
    {
        ...
        AlveyTxConverter convert(tx, active_chainstate, &mempool, NULL, NULL, contractflags);
