...
BOOST_AUTO_TEST_CASE(bytecodeexec_txs_empty){
    initState();
    std::vector<AlveyTransaction> txs;
...
BOOST_AUTO_TEST_CASE(bytecodeexec_create_contract){
    initState();
    AlveyTransaction txEth = createAlveyTransaction(CODE[0], 0, GASLIMIT, dev::u256(1), HASHTX, dev::Address());
    std::vector<AlveyTransaction> txs(1, txEth);
    auto result = executeBC(txs, *m_node.chainman);
    
    std::vector<dev::Address> addrs = {createAlveyAddress(txs[0].getHashWith(), txs[0].getNVout())};
...
BOOST_AUTO_TEST_CASE(bytecodeexec_create_contract_OutOfGasIntrinsic){
    initState();
    AlveyTransaction txEth = createAlveyTransaction(CODE[0], 0, dev::u256(100), dev::u256(1), HASHTX, dev::Address());
    std::vector<AlveyTransaction> txs(1, txEth);
    auto result = executeBC(txs, *m_node.chainman);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_create_contract_OutOfGas){
    initState();
    AlveyTransaction txEth = createAlveyTransaction(CODE[1], 0, GASLIMIT, dev::u256(1), HASHTX, dev::Address());
    std::vector<AlveyTransaction> txs(1, txEth);
    auto result = executeBC(txs, *m_node.chainman);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_OutOfGasIntrinsic_create_contract_normal_create_contract){
    ...
    for(size_t i = 0; i < 10; i++){
        ...
        AlveyTransaction txEth = createAlveyTransaction(CODE[0], 0, gasLimit, dev::u256(1), hash, dev::Address());
        newAddressGen.push_back(createAlveyAddress(txEth.getHashWith(), txEth.getNVout()));
        txs.push_back(txEth);
        ++hash;
    }
    auto result = executeBC(txs, *m_node.chainman);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_OutOfGas_create_contract_normal_create_contract){
    ...
    for(size_t i = 0; i < 10; i++){
        ...
        AlveyTransaction txEth = createAlveyTransaction(code, 0, GASLIMIT, dev::u256(1), hash, dev::Address());
        newAddressGen.push_back(createAlveyAddress(txEth.getHashWith(), txEth.getNVout()));
        txs.push_back(txEth);
        ++hash;
    }
...
BOOST_AUTO_TEST_CASE(bytecodeexec_create_contract_many){
    ...
    for(size_t i = 0; i < 130; i++){
        AlveyTransaction txEth = createAlveyTransaction(CODE[0], 0, GASLIMIT, dev::u256(1), hash, dev::Address(), i);
        newAddressGen.push_back(createAlveyAddress(txEth.getHashWith(), txEth.getNVout()));
        txs.push_back(txEth);
        ++hash;
    }
...
BOOST_AUTO_TEST_CASE(bytecodeexec_call_contract_transfer){
    initState();
    AlveyTransaction txEthCreate = createAlveyTransaction(CODE[0], 0, GASLIMIT, dev::u256(1), HASHTX, dev::Address());
    std::vector<AlveyTransaction> txsCreate(1, txEthCreate);
    executeBC(txsCreate, *m_node.chainman);
    std::vector<dev::Address> addrs = {createAlveyAddress(txsCreate[0].getHashWith(), txsCreate[0].getNVout())};
    AlveyTransaction txEthCall = createAlveyTransaction(ParseHex("00"), 1300, GASLIMIT, dev::u256(1), HASHTX, addrs[0]);
    std::vector<AlveyTransaction> txsCall(1, txEthCall);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_call_contract_transfer_OutOfGasIntrinsic_return_value){
    ...
    std::vector<AlveyTransaction> txsCreate(1, txEthCreate);
    executeBC(txsCreate, *m_node.chainman);
    dev::Address newAddress(createAlveyAddress(txsCreate[0].getHashWith(), txsCreate[0].getNVout()));
    AlveyTransaction txEthCall = createAlveyTransaction(ParseHex("00"), 1300, dev::u256(1), dev::u256(1), HASHTX, newAddress);
    std::vector<AlveyTransaction> txsCall(1, txEthCall);
    auto result = executeBC(txsCall, *m_node.chainman);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_call_contract_transfer_OutOfGas_return_value){
    initState();
    AlveyTransaction txEthCreate = createAlveyTransaction(CODE[2], 0, GASLIMIT, dev::u256(1), HASHTX, dev::Address());
    std::vector<AlveyTransaction> txsCreate(1, txEthCreate);
    executeBC(txsCreate, *m_node.chainman);
    dev::Address newAddress(createAlveyAddress(txsCreate[0].getHashWith(), txsCreate[0].getNVout()));
    AlveyTransaction txEthCall = createAlveyTransaction(ParseHex("00"), 1300, GASLIMIT, dev::u256(1), HASHTX, newAddress);
    std::vector<AlveyTransaction> txsCall(1, txEthCall);
    auto result = executeBC(txsCall, *m_node.chainman);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_call_contract_transfer_many){
    ...
    std::vector<AlveyTransaction> txsCall;
    std::vector<dev::Address> addrs;
    valtype codeCall(ParseHex("00"));
    dev::h256 hashCall(HASHTX);
    for(size_t i = 0; i < txs.size(); i++){
        AlveyTransaction txEthCall = createAlveyTransaction(codeCall, 1300, GASLIMIT, dev::u256(1), hash, newAddressGen[i], i);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_call_contract_OutOfGas_transfer_many_return_value){
    ...
    dev::h256 hash(HASHTX);
    for(size_t i = 0; i < 130; i++){
        AlveyTransaction txEth = createAlveyTransaction(CODE[2], 0, GASLIMIT, dev::u256(1), hash, dev::Address(), i);
        newAddressGen.push_back(createAlveyAddress(txEth.getHashWith(), txEth.getNVout()));
        txs.push_back(txEth);
        ++hash;
    }
    ...
    dev::h256 hashCall(HASHTX);
    for(size_t i = 0; i < txs.size(); i++){
        AlveyTransaction txEthCall = createAlveyTransaction(codeCall, 1300, GASLIMIT, dev::u256(1), hashCall, newAddressGen[i], i);
        txsCall.push_back(txEthCall);
        addrs.push_back(txEthCall.receiveAddress());
        ++hashCall;
    }
...
BOOST_AUTO_TEST_CASE(bytecodeexec_suicide){
    initState();
    std::vector<AlveyTransaction> txsCreate;
    std::vector<dev::Address> newAddresses;
    dev::h256 hash(HASHTX);
    for(size_t i = 0; i < 10; i++){
        valtype code = i == 0 ? CODE[0] : CODE[4];
        dev::u256 value = i == 0 ? dev::u256(0) : dev::u256(13);
        AlveyTransaction txEthCreate = createAlveyTransaction(code, 0, GASLIMIT, dev::u256(1), hash, dev::Address(), i);
        txsCreate.push_back(txEthCreate);

        AlveyTransaction txEthSendValue;
        if(i != 0){
            txEthSendValue = createAlveyTransaction(valtype(), value, GASLIMIT, dev::u256(1), hash, createAlveyAddress(txEthCreate.getHashWith(), txEthCreate.getNVout()), i);
            txsCreate.push_back(txEthSendValue);
        }

        newAddresses.push_back(createAlveyAddress(txEthCreate.getHashWith(), txEthCreate.getNVout()));
        ++hash;
    }
    ...
    for(size_t i = 0; i < newAddresses.size() - 1; i++){
        AlveyTransaction txEthCall = createAlveyTransaction(codeCall, 0, GASLIMIT, dev::u256(1), HASHTX, newAddresses[i + 1]);
        txsCall.push_back(txEthCall);
        addrs.push_back(txEthCall.receiveAddress());
    }
    auto result = executeBC(txsCall, *m_node.chainman);
...
BOOST_AUTO_TEST_CASE(bytecodeexec_contract_create_contracts){
    initState();
    AlveyTransaction txEthCreate = createAlveyTransaction(CODE[3], 0, GASLIMIT, dev::u256(1), HASHTX, dev::Address());
    std::vector<AlveyTransaction> txs(1, txEthCreate);
    executeBC(txs, *m_node.chainman);
    std::vector<AlveyTransaction> txsCall;
    valtype codeCall(ParseHex("3f811b80"));
    dev::Address newAddress(createAlveyAddress(txEthCreate.getHashWith(), txEthCreate.getNVout()));
    std::vector<dev::Address> addrs;
    for(size_t i = 0; i < 20; i++){
        AlveyTransaction txEthCall = createAlveyTransaction(codeCall, 0, GASLIMIT, dev::u256(1), HASHTX, newAddress, i);
        txsCall.push_back(txEthCall);
        addrs.push_back(txEthCall.receiveAddress());
    }
    auto result = executeBC(txsCall, *m_node.chainman);
