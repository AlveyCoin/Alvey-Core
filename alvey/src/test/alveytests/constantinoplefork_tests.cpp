...
//    initState();
    ...
    txs.push_back(createAlveyTransaction(CODE[0], 0, GASLIMIT, dev::u256(1), hashTx, dev::Address()));
    txs.push_back(createAlveyTransaction(CODE[1], 0, GASLIMIT, dev::u256(1), ++hashTx, dev::Address()));
    ...
    txsCall.push_back(createAlveyTransaction(CODE[2], 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
    executeBC(txsCall, *m_node.chainman);

    // Call mint
    std::vector<AlveyTransaction> txsMint;
    txsMint.push_back(createAlveyTransaction(CODE[3], 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
    ...
    // Call balance of
    std::vector<AlveyTransaction> txsbalance;
    txsbalance.push_back(createAlveyTransaction(ParseHex("70a082310000000000000000000000000101010101010101010101010101010101010101"), 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
...
//    initState();
    ...
    // Create contract
    std::vector<AlveyTransaction> txs;
    txs.push_back(createAlveyTransaction(CODE[4], 0, GASLIMIT, dev::u256(1), hashTx, dev::Address()));
    executeBC(txs, *m_node.chainman);

    // Call is it constantinople
    dev::Address proxy = createAlveyAddress(txs[0].getHashWith(), txs[0].getNVout());
    std::vector<AlveyTransaction> txIsItConstantinople;
    txIsItConstantinople.push_back(createAlveyTransaction(CODE[5], 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
