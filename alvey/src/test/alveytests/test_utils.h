#ifndef ALVTESTS_TEST_UTILS_H
#define ALVTESTS_TEST_UTILS_H
...
extern std::unique_ptr<AlveyState> globalState;
...
inline void initState(){
    ...
    const std::string dirAlvey = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<AlveyState>(new AlveyState(dev::u256(0), AlveyState::openDB(dirAlvey, hashDB, dev::WithExisting::Trust), dirAlvey + "/alveyDB", dev::eth::BaseState::Empty));
...
inline dev::Address createAlveyAddress(dev::h256 hashTx, uint32_t voutNumber){
...
inline AlveyTransaction createAlveyTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout = 0){
    AlveyTransaction txEth;
    if(recipient == dev::Address()){
        txEth = AlveyTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = AlveyTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
...
inline std::pair<std::vector<ResultExecute>, ByteCodeExecResult> executeBC(std::vector<AlveyTransaction> txs, ChainstateManager& chainman){
    CBlock block(generateBlock());
    AlveyDGP alveyDGP(globalState.get(), chainman.ActiveChainstate(), fGettingValuesDGP);
    uint64_t blockGasLimit = alveyDGP.getBlockGasLimit(chainman.ActiveChain().Tip()->nHeight + 1);
