...
AlveyState::AlveyState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = AlveyState::openDB(_path + "/alveyDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

AlveyState::AlveyState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute AlveyState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, AlveyTransaction const& _t, CChain& _chain, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createAlveyAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    if(voutLimit){
        ...
        return ResultExecute{
            ex,
            AlveyTransactionReceipt(oldStateRoot, oldUTXORoot, gas, e.logs(), {}, {}),
        ...
        if (res.excepted == dev::eth::TransactionException::None) {
            ...
            return ResultExecute{
                res,
                AlveyTransactionReceipt(rootHash(), rootHashUTXO(), startGasUsed + e.gasUsed(), e.logs(), {}, {}),
                tx ? *tx : CTransaction()
            };
        }
    }
...
std::unordered_map<dev::Address, Vin> AlveyState::vins() const // temp
{
...
void AlveyState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* AlveyState::vin(dev::Address const& _a) const
{
    return const_cast<AlveyState*>(this)->vin(_a);
}

Vin* AlveyState::vin(dev::Address const& _addr)
{
...
// void AlveyState::commit(CommitBehaviour _commitBehaviour)
// {
...
void AlveyState::kill(dev::Address _addr)
{
...
void AlveyState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
void AlveyState::deleteAccounts(std::set<dev::Address>& addrs){
...
void AlveyState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void AlveyState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
    clog(dev::VerbosityWarning, "exec") << "VM exception:" << ss.str();
}

void AlveyState::validateTransfersWithChangeLog(){
...
void AlveyState::deployDelegationsContract(){
    dev::Address delegationsAddress = uintToh160(Params().GetConsensus().delegationsAddress);
    if(!AlveyState::addressInUse(delegationsAddress)){
        AlveyState::createContract(delegationsAddress);
        AlveyState::setCode(delegationsAddress, bytes{fromHex(DELEGATIONS_CONTRACT_CODE)}, AlveyState::version(delegationsAddress));
        commit(CommitBehaviour::RemoveEmptyAccounts);
        db().commit();
    }
}
