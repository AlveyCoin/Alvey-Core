...
void AlveyDGP::initDataSchedule(){
    dataSchedule = scheduleDataForBlockNumber(0);
}

bool AlveyDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData, int blockHeight){
...
dev::eth::EVMSchedule AlveyDGP::getGasSchedule(int blockHeight){
    clear();
...
uint64_t AlveyDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t AlveyDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t AlveyDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t AlveyDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool AlveyDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void AlveyDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void AlveyDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void AlveyDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data, chainstate)[0].execRes.output;
}

void AlveyDGP::createParamsInstance(){
...
dev::Address AlveyDGP::getAddressForBlock(unsigned int blockHeight){
...
void AlveyDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void AlveyDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void AlveyDGP::parseStorageOneUint64(uint64_t& value){
...
void AlveyDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule AlveyDGP::createEVMSchedule(const dev::eth::EVMSchedule &_schedule, int blockHeight){
