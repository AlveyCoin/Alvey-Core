...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    //////////////////////////////////////////////////////// alvey
    AlveyDGP alveyDGP(globalState.get(), m_chainstate, fGettingValuesDGP);
    globalSealEngine->setAlveySchedule(alveyDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = alveyDGP.getBlockSize(nHeight);
    minGasPrice = alveyDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = alveyDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice, CBlock* pblock) {
    ...
    AlveyTxConverter convert(iter->GetTx(), m_chainstate, &m_mempool, NULL, &pblock->vtx, contractflags);

    ExtractAlveyTX resultConverter;
    if(!convert.extractionAlveyTransactions(resultConverter)){
    ...
    dev::u256 txGas = 0;
    for(AlveyTransaction alveyTransaction : alveyTransactions){
        txGas += alveyTransaction.gas();
        if(txGas > txGasLimit) {
        ...
        if(bceResult.usedGas + alveyTransaction.gas() > softBlockGasLimit){
        ...
        if(alveyTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, alveyTransactions, hardBlockGasLimit, m_chainstate.m_chain.Tip(), m_chainstate.m_chain);
...
public:
    ...
    void Update(int32_t nHeight)
    {
        ...
        if(nHeight <= checkpointSpan)
        {
            ...
            alveyDelegations.FilterDelegationEvents(events, *this, pwallet->chain().chainman());
            delegations_staker = alveyDelegations.DelegationsFromEvents(events);
        }
        else
        {
            ...
            {
                std::vector<DelegationEvent> events;
                alveyDelegations.FilterDelegationEvents(events, *this, pwallet->chain().chainman(), cacheHeight, cpsHeight);
                alveyDelegations.UpdateDelegationsFromEvents(events, cacheDelegationsStaker);
                cacheHeight = cpsHeight;
            }

            // Update the wallet delegations
            std::vector<DelegationEvent> events;
            alveyDelegations.FilterDelegationEvents(events, *this, pwallet->chain().chainman(), cacheHeight + 1);
            delegations_staker = cacheDelegationsStaker;
            alveyDelegations.UpdateDelegationsFromEvents(events, delegations_staker);
        }
...
private:
    CWallet *pwallet;
    AlveyDelegation alveyDelegations;
    int32_t cacheHeight;
    ...
    void Update(int32_t nHeight)
    {
        if(fLogEvents)
        {
            ...
            if(nHeight <= checkpointSpan)
            {
                ...
                std::vector<DelegationEvent> events;
                alveyDelegations.FilterDelegationEvents(events, *this, pwallet->chain().chainman());
                pwallet->m_my_delegations = alveyDelegations.DelegationsFromEvents(events);
            }
            else
            {
                // Update the cached delegations for the staker, older then the sync checkpoint (500 blocks)
                ...
                if(cacheHeight < cpsHeight)
                {
                    std::vector<DelegationEvent> events;
                    alveyDelegations.FilterDelegationEvents(events, *this, pwallet->chain().chainman(), cacheHeight, cpsHeight);
                    alveyDelegations.UpdateDelegationsFromEvents(events, cacheMyDelegations);
                    cacheHeight = cpsHeight;
                }
                ...
                alveyDelegations.UpdateDelegationsFromEvents(events, pwallet->m_my_delegations);
            }
        }
        else
        {
            ...
            if(cacheHeight != nHeight)
            {
                ...
                for(auto item: mapAddress)
                {
                    ...
                    if(alveyDelegations.GetDelegation(address, delegation, pwallet->chain().chainman().ActiveChainstate()) && AlveyDelegation::VerifyDelegation(address, delegation))
                    {
...
public:
...

    {
        ...
        std::string threadName = "alveystake";
...
void StakeAlveys(bool fStake, CWallet *pwallet, boost::thread_group*& stakeThread)
{
