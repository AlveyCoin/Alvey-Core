...
#if defined(NDEBUG)
# error "AlveyCoin cannot be compiled without assertions."
#endif
...
std::unique_ptr<AlveyCoinState> globalState;
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if(tx.HasCreateOrCall()){
        ...
        uint64_t minGasPrice = alveyDGP.getMinGasPrice(::ChainActive().Tip()->nHeight + 1);
        uint64_t blockGasLimit = alveyDGP.getBlockGasLimit(::ChainActive().Tip()->nHeight + 1);
        size_t count = 0;
        ...
        AlveyCoinTxConverter converter(tx, NULL, NULL, contractflags);
        ExtractAlveyCoinTX resultConverter;
        if(!converter.extractionAlveyCoinTransactions(resultConverter)){
        ...
        std::vector<AlveyCoinTransaction> alveyTransactions = resultConverter.first;
        std::vector<EthTransactionParams> alveyETP = resultConverter.second;

        dev::u256 sumGas = dev::u256(0);
        dev::u256 gasAllTxs = dev::u256(0);
        for(AlveyCoinTransaction alveyTransaction : alveyTransactions){
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit, CAmount nAmount){
    ...
    AlveyCoinTransaction callTransaction;
    if(addrContract == dev::Address())
    {
        callTransaction = AlveyCoinTransaction(nAmount, 1, dev::u256(gasLimit), opcode, nonce);
    }
    else
    {
        callTransaction = AlveyCoinTransaction(nAmount, 1, dev::u256(gasLimit), addrContract, opcode, nonce);
    }
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());

    
    ByteCodeExec exec(block, std::vector<AlveyCoinTransaction>(1, callTransaction), blockGasLimit, pblockindex);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path alveyDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
...
bool ByteCodeExec::performByteCode(dev::eth::Permanence type){
    for(AlveyCoinTransaction& tx : txs){
        //validate VM version
        ...
        if(!tx.isCreation() && !globalState->addressInUse(tx.receiveAddress())){
            ...
            result.push_back(ResultExecute{execRes, AlveyCoinTransactionReceipt(dev::h256(), dev::h256(), dev::u256(), dev::eth::LogEntries()), CTransaction()});
            continue;
        }
...
bool AlveyCoinTxConverter::extractionAlveyCoinTransactions(ExtractAlveyCoinTX& alveytx){
    ...
    std::vector<AlveyCoinTransaction> resultTX;
...
bool AlveyCoinTxConverter::receiveStack(const CScript& scriptPubKey){
    sender = false;
...
bool AlveyCoinTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
AlveyCoinTransaction AlveyCoinTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    AlveyCoinTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = AlveyCoinTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = AlveyCoinTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
...
size_t AlveyCoinTxConverter::correctedStackSize(size_t size){
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // alvey
    AlveyCoinDGP alveyDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setAlveyCoinSchedule(alveyDGP.getGasSchedule(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1) ));
    uint32_t sizeBlockDGP = alveyDGP.getBlockSize(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t minGasPrice = alveyDGP.getMinGasPrice(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t blockGasLimit = alveyDGP.getBlockGasLimit(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
...
///////////////////////////////////////////////////////////////////////////////////////// alvey
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            AlveyCoinTxConverter convert(tx, &view, &block.vtx, contractflags);

            ExtractAlveyCoinTX resultConvertAlveyCoinTX;
            if(!convert.extractionAlveyCoinTransactions(resultConvertAlveyCoinTX)){
            ...
            if(!CheckMinGasPrice(resultConvertAlveyCoinTX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertAlveyCoinTX.first, blockGasLimit, pindex->pprev);
            ...
            for(AlveyCoinTransaction& qtx : resultConvertAlveyCoinTX.first){
            ...
            if (fLogEvents && !fJustCheck)
            {
                ...
                for(size_t k = 0; k < resultConvertAlveyCoinTX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{
                        ...
                        tx.GetHash(),
                        uint32_t(i),
                        resultConvertAlveyCoinTX.first[k].from(),
                        resultConvertAlveyCoinTX.first[k].to(),
                        ...
                        resultConvertAlveyCoinTX.first[k].getNVout(),
...
bool SignBlockHWI(std::shared_ptr<CBlock> pblock, CWallet& wallet, std::vector<unsigned char>& vchSig)
{
    ...
    AlveyCoinLedger &device = AlveyCoinLedger::instance();
...
bool SignBlockLedger(std::shared_ptr<CBlock> pblock, CWallet& wallet, std::vector<unsigned char>& vchSig)
{
    ...
    {
        std::string errorMessage = AlveyCoinLedger::instance().errorMessage();
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for AlveyCoin PoS we need to sync this after every block to ensure txdb is populated for
...
CAmount GetTxGasFee(const CMutableTransaction& _tx)
{
    ...
    if(tx.HasCreateOrCall())
    {
        ...
        AlveyCoinTxConverter convert(tx, NULL, NULL, contractflags);
