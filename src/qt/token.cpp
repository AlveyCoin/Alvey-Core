...
Token::Token()
{
    ...
    lstMandatory.append(AlveyCoinToken::paramAddress());
    lstMandatory.append(AlveyCoinToken::paramDatahex());
    QStringList lstOptional;
    lstOptional.append(AlveyCoinToken::paramSender());
    ...
    lstMandatory.append(AlveyCoinToken::paramDatahex());
    lstOptional.clear();
    lstOptional.append(AlveyCoinToken::paramAmount());
    lstOptional.append(AlveyCoinToken::paramGasLimit());
    lstOptional.append(AlveyCoinToken::paramGasPrice());
    lstOptional.append(AlveyCoinToken::paramSender());
    lstOptional.append(AlveyCoinToken::paramBroadcast());
    lstOptional.append(AlveyCoinToken::paramChangeToSender());
    lstOptional.append(AlveyCoinToken::paramPsbt());
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            if(numTopics > 1)
            {
                ...
                Token::ToAlveyCoinAddress(tokenEvent.sender, tokenEvent.sender);
            }
            if(numTopics > 2)
            {
                tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
                Token::ToAlveyCoinAddress(tokenEvent.receiver, tokenEvent.receiver);
            }
