...
class AlveyCoinHwiToolPriv
{
public:
    AlveyCoinHwiToolPriv(QObject *parent)
    {
...
AlveyCoinHwiTool::AlveyCoinHwiTool(QObject *parent) : QObject(parent)
{
    d = new AlveyCoinHwiToolPriv(this);
}

AlveyCoinHwiTool::~AlveyCoinHwiTool()
{
    delete d;
}

bool AlveyCoinHwiTool::enumerate(QList<HWDevice> &devices, bool stake)
{
    LOCK(cs_ledger);
    devices.clear();
    std::vector<LedgerDevice> vecDevices;
    if(AlveyCoinLedger::instance().enumerate(vecDevices, stake))
    {
    ...
    {
        d->strError = QString::fromStdString(AlveyCoinLedger::instance().errorMessage());
    }

    return devices.size() > 0;
}

bool AlveyCoinHwiTool::isConnected(const QString &fingerprint, bool stake)
{
    LOCK(cs_ledger);
    std::string strFingerprint = fingerprint.toStdString();
    bool ret = AlveyCoinLedger::instance().isConnected(strFingerprint, stake);
    if(!ret) d->strError = QString::fromStdString(AlveyCoinLedger::instance().errorMessage());
    return ret;
}

bool AlveyCoinHwiTool::getKeyPool(const QString &fingerprint, int type, const QString& path, bool internal, QString &desc)
{
    LOCK(cs_ledger);
    std::string strFingerprint = fingerprint.toStdString();
    ...
    bool ret = AlveyCoinLedger::instance().getKeyPool(strFingerprint, type, strPath, internal, d->from, d->to, strDesc);
...
bool AlveyCoinHwiTool::getKeyPool(const QString &fingerprint, int type, const QString &path, QStringList &descs)
{
    LOCK(cs_ledger);
    bool ret = true;
    QString desc;
...
bool AlveyCoinHwiTool::getKeyPoolPKH(const QString &fingerprint, const QString& path, QStringList &descs)
{
    return getKeyPool(fingerprint, (int)OutputType::LEGACY, path, descs);
}

bool AlveyCoinHwiTool::getKeyPoolP2SH(const QString &fingerprint, const QString& path, QStringList &descs)
{
    return getKeyPool(fingerprint, (int)OutputType::P2SH_SEGWIT, path, descs);
}

bool AlveyCoinHwiTool::getKeyPoolBech32(const QString &fingerprint, const QString& path, QStringList &descs)
{
    return getKeyPool(fingerprint, (int)OutputType::BECH32, path, descs);
}

bool AlveyCoinHwiTool::signTx(const QString &fingerprint, QString &psbt)
{
    LOCK(cs_ledger);
    std::string strFingerprint = fingerprint.toStdString();
    std::string strPsbt = psbt.toStdString();
    bool ret = AlveyCoinLedger::instance().signTx(strFingerprint, strPsbt);
    psbt = QString::fromStdString(strPsbt);
    if(!ret) d->strError = QString::fromStdString(AlveyCoinLedger::instance().errorMessage());
    return ret;
}

bool AlveyCoinHwiTool::signMessage(const QString &fingerprint, const QString &message, const QString &path, QString &signature)
{
    LOCK(cs_ledger);
    std::string strFingerprint = fingerprint.toStdString();
    ...
    bool ret = AlveyCoinLedger::instance().signMessage(strFingerprint, strMessage, strPath, strSignature);
    signature = QString::fromStdString(strSignature);
    if(!ret) d->strError = QString::fromStdString(AlveyCoinLedger::instance().errorMessage());
    return ret;
}

bool AlveyCoinHwiTool::signDelegate(const QString &fingerprint, QString &psbt)
{
...
QString AlveyCoinHwiTool::errorMessage()
{
...
bool AlveyCoinHwiTool::isStarted()
{
    return d->fStarted;
}

void AlveyCoinHwiTool::wait()
{
    if(d->fStarted)
    {
...
bool AlveyCoinHwiTool::rescanBlockchain(int startHeight, int stopHeight)
{
    if(!d->model) return false;
...
bool AlveyCoinHwiTool::importAddresses(const QString &desc)
{
    if(!d->model) return false;
...
bool AlveyCoinHwiTool::importMulti(const QStringList &descs)
{
    bool ret = true;
...
bool AlveyCoinHwiTool::finalizePsbt(const QString &psbt, QString &hexTx, bool &complete)
{
    if(!d->model) return false;
...
bool AlveyCoinHwiTool::sendRawTransaction(const QString &hexTx, QVariantMap& variantMap)
{
    if(!d->model) return false;
...
bool AlveyCoinHwiTool::decodePsbt(const QString &psbt, QString &decoded)
{
    if(!d->model) return false;
...
void AlveyCoinHwiTool::setModel(WalletModel *model)
{
    d->model = model;
}

bool AlveyCoinHwiTool::execRPC(ExecRPCCommand *cmd, const QMap<QString, QString> &lstParams, QVariant &result, QString &resultJson)
{
    d->strError.clear();
...
void AlveyCoinHwiTool::addError(const QString &error)
{
    if(d->strError != "")
        d->strError += "\n";
    d->strError += error;
...
QString AlveyCoinHwiTool::derivationPathPKH()
{
    std::string path = AlveyCoinLedger::instance().derivationPath((int)OutputType::LEGACY);
    return QString::fromStdString(path);
}

QString AlveyCoinHwiTool::derivationPathP2SH()
{
    std::string path = AlveyCoinLedger::instance().derivationPath((int)OutputType::P2SH_SEGWIT);
    return QString::fromStdString(path);
}

QString AlveyCoinHwiTool::derivationPathBech32()
{
    std::string path = AlveyCoinLedger::instance().derivationPath((int)OutputType::BECH32);
    return QString::fromStdString(path);
}
