...
void AlveyCoinDGP::initDataSchedule(){
    dataSchedule = scheduleDataForBlockNumber(0);
}

bool AlveyCoinDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData, int blockHeight){
...
dev::eth::EVMSchedule AlveyCoinDGP::getGasSchedule(int blockHeight){
    clear();
...
uint64_t AlveyCoinDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t AlveyCoinDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t AlveyCoinDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t AlveyCoinDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool AlveyCoinDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void AlveyCoinDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void AlveyCoinDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void AlveyCoinDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void AlveyCoinDGP::createParamsInstance(){
...
dev::Address AlveyCoinDGP::getAddressForBlock(unsigned int blockHeight){
...
void AlveyCoinDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void AlveyCoinDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void AlveyCoinDGP::parseStorageOneUint64(uint64_t& value){
...
void AlveyCoinDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule AlveyCoinDGP::createEVMSchedule(const dev::eth::EVMSchedule &_schedule, int blockHeight){
