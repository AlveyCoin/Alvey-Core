...
AlveyCoinState::AlveyCoinState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = AlveyCoinState::openDB(_path + "/alveyDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

AlveyCoinState::AlveyCoinState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute AlveyCoinState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, AlveyCoinTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createAlveyCoinAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            alvey::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
    ...
    if(voutLimit){
        ...
        return ResultExecute{ex, AlveyCoinTransactionReceipt(oldStateRoot, oldUTXORoot, gas, e.logs()), refund.vout.empty() ? CTransaction() : CTransaction(refund)};
    }else{
        return ResultExecute{res, AlveyCoinTransactionReceipt(rootHash(), rootHashUTXO(), startGasUsed + e.gasUsed(), e.logs()), tx ? *tx : CTransaction()};
    }
}

std::unordered_map<dev::Address, Vin> AlveyCoinState::vins() const // temp
{
...
void AlveyCoinState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* AlveyCoinState::vin(dev::Address const& _a) const
{
    return const_cast<AlveyCoinState*>(this)->vin(_a);
}

Vin* AlveyCoinState::vin(dev::Address const& _addr)
{
...
// void AlveyCoinState::commit(CommitBehaviour _commitBehaviour)
// {
...
void AlveyCoinState::kill(dev::Address _addr)
{
...
void AlveyCoinState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
void AlveyCoinState::deleteAccounts(std::set<dev::Address>& addrs){
...
void AlveyCoinState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void AlveyCoinState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
    clog(dev::VerbosityWarning, "exec") << "VM exception:" << ss.str();
}

void AlveyCoinState::validateTransfersWithChangeLog(){
...
void AlveyCoinState::deployDelegationsContract(){
    dev::Address delegationsAddress = uintToh160(Params().GetConsensus().delegationsAddress);
    if(!AlveyCoinState::addressInUse(delegationsAddress)){
        AlveyCoinState::createContract(delegationsAddress);
        AlveyCoinState::setCode(delegationsAddress, bytes{fromHex(DELEGATIONS_CONTRACT_CODE)}, AlveyCoinState::version(delegationsAddress));
        commit(CommitBehaviour::RemoveEmptyAccounts);
        db().commit();
    }
}
