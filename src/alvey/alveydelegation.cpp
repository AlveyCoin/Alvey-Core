...
class AlveyCoinDelegationPriv
{
public:
    AlveyCoinDelegationPriv():
        m_pfDelegations(0),
    ...
    virtual ~AlveyCoinDelegationPriv()
    {
        if(m_pfDelegations)
...
AlveyCoinDelegation::AlveyCoinDelegation():
    priv(0)
{
    priv = new AlveyCoinDelegationPriv();
}
...
bool AlveyCoinDelegation::GetDelegation(const uint160 &address, Delegation &delegation) const
{
...
bool AlveyCoinDelegation::VerifyDelegation(const uint160 &address, const Delegation &delegation)
{
...
bool AlveyCoinDelegation::FilterDelegationEvents(std::vector<DelegationEvent> &events, const IDelegationFilter &filter, int fromBlock, int toBlock, int minconf) const
{
...
std::map<uint160, Delegation> AlveyCoinDelegation::DelegationsFromEvents(const std::vector<DelegationEvent> &events)
{
...
void AlveyCoinDelegation::UpdateDelegationsFromEvents(const std::vector<DelegationEvent> &events, std::map<uint160, Delegation> &delegations)
{
...
bool AlveyCoinDelegation::ExistDelegationContract() const
{
    // Delegation contract exist check
    return globalState && globalState->addressInUse(priv->delegationsAddress);
}

std::string AlveyCoinDelegation::BytecodeRemove()
{
    return DelegationABI()["removeDelegation"].selector();
}

bool AlveyCoinDelegation::BytecodeAdd(const std::string &hexStaker, const int &fee, const std::vector<unsigned char> &PoD, std::string &datahex, std::string &errorMessage)
{
...
bool AlveyCoinDelegation::IsAddBytecode(const std::vector<unsigned char> &data)
{
...
bool AlveyCoinDelegation::GetUnsignedStaker(const std::vector<unsigned char> &data, std::string &hexStaker)
{
...
bool AlveyCoinDelegation::SetSignedStaker(std::vector<unsigned char> &data, const std::string &base64PoD)
{
    if(!IsAddBytecode(data))
        return false;
