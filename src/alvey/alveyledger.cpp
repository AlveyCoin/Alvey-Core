...
using namespace AlveyCoinLedger_NS;

class AlveyCoinLedgerPriv
{
public:
    AlveyCoinLedgerPriv()
    {
        ...
        if(!toolExists)
        {
            LogPrintf("AlveyCoinLedger(): HWI tool not found %s\n", toolPath);
        }
    }
...
AlveyCoinLedger::AlveyCoinLedger():
    d(0)
{
    d = new AlveyCoinLedgerPriv();
}
...
AlveyCoinLedger &AlveyCoinLedger::instance()
{
    static AlveyCoinLedger device;
    return device;
}

bool AlveyCoinLedger::signCoinStake(const std::string &fingerprint, std::string &psbt)
{
    LOCK(cs_ledger);
...
bool AlveyCoinLedger::signBlockHeader(const std::string &fingerprint, const std::string &header, const std::string &path, std::vector<unsigned char> &vchSig)
{
    LOCK(cs_ledger);
    // Check if tool exists
    if(!toolExists())
...
bool AlveyCoinLedger::isConnected(const std::string &fingerprint, bool stake)
{
...
bool AlveyCoinLedger::enumerate(std::vector<LedgerDevice> &devices, bool stake)
{
    LOCK(cs_ledger);
    // Check if tool exists
    if(!toolExists())
...
bool AlveyCoinLedger::signTx(const std::string &fingerprint, std::string &psbt)
{
    LOCK(cs_ledger);
    // Check if tool exists
    if(!toolExists())
...
bool AlveyCoinLedger::signMessage(const std::string &fingerprint, const std::string &message, const std::string &path, std::string &signature)
{
    LOCK(cs_ledger);
    // Check if tool exists
    if(!toolExists())
...
bool AlveyCoinLedger::getKeyPool(const std::string &fingerprint, int type, const std::string& path, bool internal, int from, int to, std::string &desc)
{
    LOCK(cs_ledger);
    // Check if tool exists
    if(!toolExists())
...
std::string AlveyCoinLedger::errorMessage()
{
    LOCK(cs_ledger);
...
bool AlveyCoinLedger::toolExists()
{
    return d->toolExists;
}

bool AlveyCoinLedger::isStarted()
{
    return d->fStarted;
}

void AlveyCoinLedger::wait()
{
    if(d->fStarted)
    {
...
bool AlveyCoinLedger::beginSignTx(const std::string &fingerprint, std::string &psbt)
{
...
bool AlveyCoinLedger::endSignTx(const std::string &, std::string &psbt)
{
...
bool AlveyCoinLedger::beginSignBlockHeader(const std::string &fingerprint, const std::string &header, const std::string &path, std::vector<unsigned char> &)
{
    // Execute command line
    std::vector<std::string> arguments = d->arguments;
...
bool AlveyCoinLedger::endSignBlockHeader(const std::string &, const std::string &, const std::string &, std::vector<unsigned char> &vchSig)
{
    // Decode command line results
    UniValue jsonDocument = json_read_doc(d->strStdout);
    UniValue data = json_get_object(jsonDocument);
...
bool AlveyCoinLedger::beginEnumerate(std::vector<LedgerDevice> &)
{
    // Execute command line
    std::vector<std::string> arguments = d->arguments;
    arguments << "enumerate";
...
bool AlveyCoinLedger::endEnumerate(std::vector<LedgerDevice> &devices, bool stake)
{
    // Decode command line results
    UniValue jsonDocument = json_read_doc(d->strStdout);
    ...
    for(size_t i = 0; i < jsonDevices.size(); i++)
    {
        ...
        bool isStakeApp = device.app_name == "AlveyCoin Stake" || device.app_name == "AlveyCoin Stake Test";
        if(isStakeApp == stake)
        {
...
bool AlveyCoinLedger::beginSignMessage(const std::string &fingerprint, const std::string &message, const std::string &path, std::string &)
{
    // Execute command line
    std::vector<std::string> arguments = d->arguments;
...
bool AlveyCoinLedger::endSignMessage(const std::string &, const std::string &, const std::string &, std::string &signature)
{
    // Decode command line results
    UniValue jsonDocument = json_read_doc(d->strStdout);
    UniValue data = json_get_object(jsonDocument);
...
bool AlveyCoinLedger::beginGetKeyPool(const std::string &fingerprint, int type, const std::string& path, bool internal, int from, int to, std::string &)
{
    // Get the output type
    std::string descType;
    switch (type) {
...
bool AlveyCoinLedger::endGetKeyPool(const std::string &, int, const std::string& , bool, int, int, std::string &desc)
{
    // Decode command line results
...
std::string AlveyCoinLedger::derivationPath(int type)
{
