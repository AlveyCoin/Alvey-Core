...
public:
    ...
    uint32_t nBits;
    uint32_t nNonce;
    uint256 hashStateRoot; // alvey
    uint256 hashUTXORoot; // alvey
    ...
    inline void SerializationOp(Stream& s, Operation ser_action) {
        ...
        READWRITE(nBits);
        READWRITE(nNonce);
        READWRITE(hashStateRoot); // alvey
        READWRITE(hashUTXORoot); // alvey
    ...
    void SetNull()
    {
        ...
        nBits = 0;
        nNonce = 0;
        hashStateRoot.SetNull(); // alvey
        hashUTXORoot.SetNull(); // alvey
    ...
    virtual bool IsProofOfStake() const //alvey
    {
    ...
    CBlockHeader& operator=(const CBlockHeader& other) //alvey
    {
        if (this != &other)
        {
...
public:
    ...
    std::pair<COutPoint, unsigned int> GetProofOfStake() const //alvey
    {
    ...
    CBlockHeader GetBlockHeader() const
    {
        ...
        block.nBits          = nBits;
        block.nNonce         = nNonce;
        block.hashStateRoot  = hashStateRoot; // alvey
        block.hashUTXORoot   = hashUTXORoot; // alvey
