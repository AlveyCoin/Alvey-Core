...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setAlveyCoinSchedule(alveyDGP.getGasSchedule(nHeight));
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    AlveyCoinTxConverter convert(iter->GetTx(), NULL, &pblock->vtx, contractflags);

    ExtractAlveyCoinTX resultConverter;
    if(!convert.extractionAlveyCoinTransactions(resultConverter)){
    ...
    ByteCodeExec exec(*pblock, alveyTransactions, hardBlockGasLimit, ::ChainActive().Tip());
...
public:
    ...
    void Update(int32_t nHeight)
    {
        ...
        if(nHeight <= checkpointSpan)
        {
            ...
            alveyDelegations.FilterDelegationEvents(events, *this);
        ...
        else
        {
            ...
            {
                std::vector<DelegationEvent> events;
                alveyDelegations.FilterDelegationEvents(events, *this, cacheHeight, cpsHeight);
            ...
            std::vector<DelegationEvent> events;
            alveyDelegations.FilterDelegationEvents(events, *this, cacheHeight + 1);
...
private:
    CWallet *pwallet;
    AlveyCoinDelegation alveyDelegations;
    int32_t cacheHeight;
        ...
        else
        {
            ...
            if(cacheHeight != nHeight)
            {
                ...
                for(auto item: mapAddress)
                {
                    ...
                    if(alveyDelegations.GetDelegation(address, delegation) && AlveyCoinDelegation::VerifyDelegation(address, delegation))
                    {
...
void StakeAlveyCoins(bool fStake, CWallet *pwallet, CConnman* connman, boost::thread_group*& stakeThread)
{
