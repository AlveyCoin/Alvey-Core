...
void DeleteBlockChainData()
{
    ...
    fs::remove_all(GetDataDir() / "stateAlveyCoin");
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                fs::path alveyStateDir = GetDataDir() / "stateAlveyCoin";
                bool fStatus = fs::exists(alveyStateDir);
                const std::string dirAlveyCoin(alveyStateDir.string());
                const dev::h256 hashDB(dev::sha3(dev::rlp("")));
                dev::eth::BaseState existsAlveyCoinstate = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
                globalState = std::unique_ptr<AlveyCoinState>(new AlveyCoinState(dev::u256(0), AlveyCoinState::openDB(dirAlveyCoin, hashDB, dev::WithExisting::Trust), dirAlveyCoin, existsAlveyCoinstate));
            ...
            try {
                ...
                globalSealEngine->setAlveyCoinSchedule(alveyDGP.getGasSchedule(::ChainActive().Height() + (::ChainActive().Height()+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1) ));
