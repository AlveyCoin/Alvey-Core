...
UniValue blockheaderToJSON(const CBlockIndex* tip, const CBlockIndex* blockindex)
{
    ...
    result.pushKV("hashStateRoot", blockindex->hashStateRoot.GetHex()); // alvey
    result.pushKV("hashUTXORoot", blockindex->hashUTXORoot.GetHex()); // alvey

    if(blockindex->IsProofOfStake()){
        result.pushKV("prevoutStakeHash", blockindex->prevoutStake.hash.GetHex()); // alvey
        result.pushKV("prevoutStakeVoutN", (int64_t)blockindex->prevoutStake.n); // alvey
    }
...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* tip, const CBlockIndex* blockindex, bool txDetails)
{
    ...
    result.pushKV("hashStateRoot", block.hashStateRoot.GetHex()); // alvey
    result.pushKV("hashUTXORoot", block.hashUTXORoot.GetHex()); // alvey
...
static std::vector<RPCResult> MempoolEntryDescription() { return {
    ...
    RPCResult{RPCResult::Type::NUM, "size", "(DEPRECATED) same as vsize. Only returned if alveyd is started with -deprecatedrpc=size\n"
...
UniValue getdelegationinfoforaddress(const JSONRPCRequest& request)
{
            RPCHelpMan{"getdelegationinfoforaddress",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The alvey address string"},
                },
                RPCResult{
                    RPCResult::Type::OBJ, "", "",
                    {
    ...
    AlveyCoinDelegation alveyDelegation;
    Delegation delegation;
    uint160 address = uint160(*pkhash);
    if(!alveyDelegation.GetDelegation(address, delegation)) {
        throw JSONRPCError(RPC_INTERNAL_ERROR, "Failed to get delegation");
    }
    bool verified = alveyDelegation.VerifyDelegation(address, delegation);
...
UniValue getdelegationsforstaker(const JSONRPCRequest& request)
{
            RPCHelpMan{"getdelegationsforstaker",
                ...
                {
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO, "The alvey address string for staker"},
                },
               RPCResult{
            RPCResult::Type::ARR, "", "",
                {
    ...
    if(!alveyDelegation.FilterDelegationEvents(events, filter)) {
        throw JSONRPCError(RPC_INTERNAL_ERROR, "Failed to get delegations for staker");
    }
    std::map<uint160, Delegation> delegations = alveyDelegation.DelegationsFromEvents(events);
                        ...
                        {RPCResult::Type::OBJ, "scriptPubKey", "",
                            {
                                ...
                                {RPCResult::Type::ARR, "addresses", "array of alvey addresses",
                                    {{RPCResult::Type::STR, "address", "alvey address"}}},
                            }},
...
static UniValue arc20name(const JSONRPCRequest& request)
{
            RPCHelpMan{"arc20name",
...
static UniValue arc20symbol(const JSONRPCRequest& request)
{
            RPCHelpMan{"arc20symbol",
...
static UniValue arc20totalsupply(const JSONRPCRequest& request)
{
            RPCHelpMan{"arc20totalsupply",
...
static UniValue arc20decimals(const JSONRPCRequest& request)
{
            RPCHelpMan{"arc20decimals",
...
static UniValue arc20balanceof(const JSONRPCRequest& request)
{
            RPCHelpMan{"arc20balanceof",
                "\nReturns the token balance for address\n",
                {
                    {"contractaddress", RPCArg::Type::STR_HEX, RPCArg::Optional::NO, "The contract address"},
                    {"address", RPCArg::Type::STR, RPCArg::Optional::NO,  "The alvey address to check token balance"},
                },
                RPCResult{
...
static UniValue arc20allowance(const JSONRPCRequest& request)
{
            RPCHelpMan{"arc20allowance",
                "\nReturns remaining tokens allowed to spend for an address\n",
                {
                    {"contractaddress", RPCArg::Type::STR_HEX, RPCArg::Optional::NO, "The contract address"},
                    {"addressFrom", RPCArg::Type::STR, RPCArg::Optional::NO,  "The alvey address of the account owning tokens"},
                    {"addressTo", RPCArg::Type::STR, RPCArg::Optional::NO,  "The alvey address of the account able to transfer the tokens"},
                },
                RPCResult{
...
static UniValue arc20listtransactions(const JSONRPCRequest& request)
{
            RPCHelpMan{"arc20listtransactions",
                "\nReturns transactions history for a specific address.\n",
                {
                    {"contractaddress", RPCArg::Type::STR_HEX, RPCArg::Optional::NO, "The contract address."},
                    {"addresss", RPCArg::Type::STR, RPCArg::Optional::NO,  "The alvey address to get history for."},
            ...
            RPCResult::Type::ARR, "", "",
                {
                    {RPCResult::Type::OBJ, "", "",
                        {
                            {RPCResult::Type::STR, "receiver", "The receiver alvey address"},
                            {RPCResult::Type::STR, "sender", "The sender alvey address"},
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    { "blockchain",         "arc20name",              &arc20name,              {"address"} },
    { "blockchain",         "arc20symbol",            &arc20symbol,            {"address"} },
    { "blockchain",         "arc20totalsupply",       &arc20totalsupply,       {"address"} },
    { "blockchain",         "arc20decimals",          &arc20decimals,          {"address"} },
    { "blockchain",         "arc20balanceof",         &arc20balanceof,         {"contractaddress", "address"} },
    { "blockchain",         "arc20allowance",         &arc20allowance,         {"contractaddress", "addressFrom", "addressTo"} },
    { "blockchain",         "arc20listtransactions",  &arc20listtransactions,  {"contractaddress", "address", "startblock", "minconf"} },
