...
UniValue transactionReceiptToJSON(const AlveyCoinTransactionReceipt& txRec)
{
    UniValue result(UniValue::VOBJ);
...
UniValue CallToContract(const UniValue& params)
{
    ...
    if(params.size() >= 3){
        CTxDestination alveySenderAddress = DecodeDestination(params[2].get_str());
        if (IsValidDestination(alveySenderAddress)) {
            const PKHash *keyid = boost::get<PKHash>(&alveySenderAddress);
    ...
    for(size_t i = 0; i < list.size(); i++)
    {
        ...
        for(size_t i = 0; i < listLog.size(); i++)
        {
            ...
            if(numTopics > 1)
            {
                ...
                ToAlveyCoinAddress(tokenEvent.sender, tokenEvent.sender);
            }
            if(numTopics > 2)
            {
                tokenEvent.receiver = topicsList[2].get_str().substr(24);
                ToAlveyCoinAddress(tokenEvent.receiver, tokenEvent.receiver);
            }
