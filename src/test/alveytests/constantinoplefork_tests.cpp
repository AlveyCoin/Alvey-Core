...
//    initState();
    ...
    txs.push_back(createAlveyCoinTransaction(CODE[0], 0, GASLIMIT, dev::u256(1), hashTx, dev::Address()));
    txs.push_back(createAlveyCoinTransaction(CODE[1], 0, GASLIMIT, dev::u256(1), ++hashTx, dev::Address()));
    executeBC(txs);

    // Call upgrade to
    dev::Address proxy = createAlveyCoinAddress(txs[0].getHashWith(), txs[0].getNVout());
    std::vector<AlveyCoinTransaction> txsCall;
    txsCall.push_back(createAlveyCoinTransaction(CODE[2], 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
    executeBC(txsCall);

    // Call mint
    std::vector<AlveyCoinTransaction> txsMint;
    txsMint.push_back(createAlveyCoinTransaction(CODE[3], 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
    auto result = executeBC(txsMint);
    BOOST_CHECK(result.first[0].execRes.excepted == dev::eth::TransactionException::None);

    // Call balance of
    std::vector<AlveyCoinTransaction> txsbalance;
    txsbalance.push_back(createAlveyCoinTransaction(ParseHex("70a082310000000000000000000000000101010101010101010101010101010101010101"), 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
...
//    initState();
    ...
    // Create contract
    std::vector<AlveyCoinTransaction> txs;
    txs.push_back(createAlveyCoinTransaction(CODE[4], 0, GASLIMIT, dev::u256(1), hashTx, dev::Address()));
    executeBC(txs);

    // Call is it constantinople
    dev::Address proxy = createAlveyCoinAddress(txs[0].getHashWith(), txs[0].getNVout());
    std::vector<AlveyCoinTransaction> txIsItConstantinople;
    txIsItConstantinople.push_back(createAlveyCoinTransaction(CODE[5], 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
