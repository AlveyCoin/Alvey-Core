...
void checkResult(bool isCreation, std::vector<AlveyCoinTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    AlveyCoinTxConverter converter(transaction, NULL);
    ExtractAlveyCoinTX alveyTx;
    BOOST_CHECK(converter.extractionAlveyCoinTransactions(alveyTx));
    std::vector<AlveyCoinTransaction> result = alveyTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(alveytxconverter_tests, TestingSetup)
