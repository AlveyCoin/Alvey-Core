...
extern std::unique_ptr<AlveyCoinState> globalState;
...
inline void initState(){
    ...
    const std::string dirAlveyCoin = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<AlveyCoinState>(new AlveyCoinState(dev::u256(0), AlveyCoinState::openDB(dirAlveyCoin, hashDB, dev::WithExisting::Trust), dirAlveyCoin + "/alveyDB", dev::eth::BaseState::Empty));
...
inline dev::Address createAlveyCoinAddress(dev::h256 hashTx, uint32_t voutNumber){
...
inline AlveyCoinTransaction createAlveyCoinTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout = 0){
    AlveyCoinTransaction txEth;
    if(recipient == dev::Address()){
        txEth = AlveyCoinTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = AlveyCoinTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
...
inline std::pair<std::vector<ResultExecute>, ByteCodeExecResult> executeBC(std::vector<AlveyCoinTransaction> txs){
