...
const char * const BITCOIN_CONF_FILENAME = "alvey.conf";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\AlveyCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\AlveyCoin
    // Mac: ~/Library/Application Support/AlveyCoin
    // Unix: ~/.alvey
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AlveyCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/AlveyCoin";
#else
    // Unix
    return pathRet / ".alvey";
#endif
#endif
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (copyright_devs.find("AlveyCoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The AlveyCoin Core Developers";
    }
